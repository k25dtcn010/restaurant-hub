...
import { db, dishModifiers, eq, modifierGroups, modifiers } from "@/db"
...
describe("Modifiers Router - T024: modifiers.delete", () => {
  ...
  afterAll(async () => {
    ...
    await db.delete(dishModifiers).where(eq(dishModifiers.modifierId, testModifierWithDishId))
...
describe("Modifiers Router - T029-T030: Dish-Modifier Assignment", () => {
  ...
  afterAll(async () => {
    ...
    await db.delete(dishModifiers).where(eq(dishModifiers.modifierId, testModifierId))
    ...
    test("T029-1: Manager can assign modifier to dish with group", async () => {
      ...
      const assignment = await db.query.dishModifiers.findFirst({
        where: (dishModifiers, { and, eq }) =>
          and(eq(dishModifiers.dishId, testDishId), eq(dishModifiers.modifierId, testModifierId)),
      })
  ...
  describe("T030: modifiers.getByDish", () => {
    ...
    test("T030-2: Returns empty array for dish with no modifiers", async () => {
      ...
      // Clean up
      await db
        .delete((await import("@/db")).dishes)
        .where(eq((await import("@/db")).dishes.id, newDish.id))
    })
  })
})

...
import { db, dishes, eq, ingredients, orders, payments, recipes, tables } from "@/db"
...
describe("Integration: Complete Payment Flow (T113)", () => {
  ...
  beforeEach(async () => {
    ...
    await db.update(ingredients).set({ quantity: 500 }).where(eq(ingredients.id, testIngredientId))

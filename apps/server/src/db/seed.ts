...
async function seed() {
  ...
  const insertedIngredients = await db.insert(ingredients).values(ingredientsData).returning()
  ...
  const getIngredientId = (name: string) => insertedIngredients.find((i) => i.name === name)?.id!
  const getDishId = (name: string) => insertedDishes.find((d) => d.name === name)?.id!

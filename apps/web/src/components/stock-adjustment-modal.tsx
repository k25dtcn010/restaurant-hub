...
export function StockAdjustmentModal({
	...
	const adjustStockMutation = useMutation({
		...
		onSuccess: (data) => {
			...
			queryClient.invalidateQueries({
				predicate: (query) => query.queryKey[0] === "inventory.getAll",
			});
	...
	const newQuantity = adjustment ? ingredient.quantity + parseFloat(adjustment) : ingredient.quantity;
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent>
				<DialogHeader>
					...
					<DialogDescription>
						Update the quantity of this ingredient. Use positive numbers to add stock,
						negative numbers to reduce stock.
					</DialogDescription>
				</DialogHeader>

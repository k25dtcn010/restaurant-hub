...
import { z } from "zod"
import { and, eq, gte, lte, orders, payments, sql, tables } from "@learn-bettert/db"
  ...
  getHistory: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (conditions.length > 0 || tableId) {
        const whereConditions = tableId ? [...conditions, eq(orders.tableId, tableId)] : conditions
      ...
      const results = await paymentsQuery.orderBy(payments.paidAt).limit(limit).offset(offset)

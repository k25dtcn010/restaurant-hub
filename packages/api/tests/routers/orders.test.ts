...
describe("Orders Router - orders.submit", () => {
  ...
  beforeEach(async () => {
    ...
    await db.update(ingredients).set({ quantity: 30 }).where(eq(ingredients.id, testIngredientId))
  ...
  test("should return error when order does not exist", async () => {
    ...
    await expect(caller.orders.submit({ orderId: 99999 })).rejects.toThrow()
  })

  test("should return error when insufficient stock", async () => {
    // Set stock too low
    await db.update(ingredients).set({ quantity: 0.1 }).where(eq(ingredients.id, testIngredientId))

    const caller = appRouter.createCaller(mockContext)

    await expect(caller.orders.submit({ orderId: testOrderId })).rejects.toThrow()
  })
...
describe("Orders Router - orders.getKitchenOrders (T060)", () => {
  ...
  beforeEach(async () => {
    ...
    await db.update(ingredients).set({ quantity: 20 }).where(eq(ingredients.id, testIngredientId))
    ...
    await db.update(orders).set({ status: "InKitchen" }).where(eq(orders.id, result2.orderId))
  ...
  test("should return orders with kitchen statuses (Pending, InKitchen, ReadyToServe)", async () => {
    ...
    const testOrders = result.orders.filter(
      (o: any) => o.id === pendingOrderId || o.id === inKitchenOrderId
    )
...
describe("Orders Router - orders.updateStatus (T061)", () => {
  ...
  beforeEach(async () => {
    ...
    // Reset ingredient stock
    await db.update(ingredients).set({ quantity: 15 }).where(eq(ingredients.id, testIngredientId))
...
describe("Orders Router - orders.getById (T067)", () => {
  ...
  beforeEach(async () => {
    ...
    // Reset ingredient stock
    await db.update(ingredients).set({ quantity: 25 }).where(eq(ingredients.id, testIngredientId))
  ...
  test("should return error when order does not exist", async () => {
    ...
    await expect(caller.orders.getById({ orderId: 99999 })).rejects.toThrow()
  })
})
...
describe("Orders Router - status transitions to Served/Completed (T086)", () => {
  ...
  test("should create status history for Served transition", async () => {
    ...
    const servedHistory = orderDetails.statusHistory.find((h) => h.status === "Served")

...
import { db, dishes, eq, ingredients, recipes } from "@learn-bettert/db"
...
describe("Inventory Router - inventory.getAll (T095)", () => {
  ...
  test("should sort low-stock items first", async () => {
    ...
    if (lowStockIndex !== -1 && normalStockIndex !== -1) {
      ...
      expect(lowStockIndex).toBeLessThanOrEqual(
        result.ingredients.filter((i) => i.isLowStock).length
      )
    }
  })

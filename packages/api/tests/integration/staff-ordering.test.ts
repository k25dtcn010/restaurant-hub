...
import { db, dishes, eq, ingredients, orders, recipes, tables, user } from "@learn-bettert/db"
...
describe("Staff-Assisted Ordering - T075 & T076", () => {
  ...
  test("T075: authenticated waiter can create orders using orders.create", async () => {
    ...
    expect(createdOrder!.orderItems[0].specialInstructions).toBe("Extra cheese - ordered by staff")
  })

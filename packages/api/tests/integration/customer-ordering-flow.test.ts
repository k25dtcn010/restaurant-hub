...
import { db, dishes, eq, ingredients, orderItems, orders, recipes, tables } from "@learn-bettert/db"
...
describe("Integration: Complete Customer Ordering Flow", () => {
  ...
  beforeEach(async () => {
    ...
    await db.update(ingredients).set({ quantity: 50 }).where(eq(ingredients.id, testIngredient1Id))
    await db.update(ingredients).set({ quantity: 100 }).where(eq(ingredients.id, testIngredient2Id))
  })
  ...
  test("End-to-End: Customer scans QR, views menu, creates order, adds items, and submits", async () => {
    ...
    console.log(
      `✅ Order updated, Total: $${updatedOrder.totalAmount / 100}, Items: ${updatedOrder.itemCount}`
    )
  ...
  test("End-to-End: Multiple customers ordering simultaneously don't corrupt inventory", async () => {
    ...
    await db.update(ingredients).set({ quantity: 1.0 }).where(eq(ingredients.id, testIngredient1Id))
